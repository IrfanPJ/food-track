...
function downloadTemplate() {
  ...
  XLSX.writeFile(wb, 'green-power-gym-import-template.xlsx')
}
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/members" className="p-2 rounded-xl hover:bg-[#1e293b] text-slate-400 hover:text-white transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-white">Import Members</h1>
          <p className="text-slate-400 text-sm">Import existing members from Excel or CSV</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-2">
        {[{n: 1, label: 'Download Template'}, {n: 2, label: 'Upload File'}, {n: 3, label: 'Preview & Import'}].map((s, i) => (
          <div key={s.n} className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold ${step === 'done' || (step === 'preview' && s.n <= 2) || (step === 'upload' && s.n <= 1) ? 'bg-green-500 text-white' : 'bg-[#1e293b] border border-[#334155] text-slate-400'}`}>
              {s.n}
            </div>
            <span className="text-slate-400 text-sm">{s.label}</span>
            {i < 2 && <div className="w-8 h-px bg-[#334155]" />}
          </div>
        ))}
      </div>
      ...
      <div className="bg-[#1e293b] rounded-2xl border border-[#334155] p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-white font-semibold">Step 1: Download Template</h2>
            <p className="text-slate-400 text-sm">Download and fill in the Excel template</p>
          </div>
        </div>
        <div className="bg-[#0f172a] rounded-xl p-4 mb-4">
          <p className="text-slate-400 text-xs font-medium mb-2">Template columns:</p>
          <div className="flex flex-wrap gap-2">
            {TEMPLATE_COLUMNS.map(col => (
              <span key={col} className="text-xs bg-[#1e293b] border border-[#334155] text-slate-300 px-2 py-1 rounded-lg">{col}</span>
            ))}
          </div>
        </div>
      ...
      <div className="bg-[#1e293b] rounded-2xl border border-[#334155] p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-white font-semibold">Step 2: Upload Your File</h2>
            <p className="text-slate-400 text-sm">Upload your filled Excel (.xlsx) or CSV file</p>
          </div>
        </div>
        <input ref={fileRef} type="file" accept=".xlsx,.csv,.xls" onChange={handleFile} className="hidden" />
        <button
          onClick={() => fileRef.current?.click()}
          className="w-full border-2 border-dashed border-[#334155] hover:border-green-500/50 rounded-xl p-8 text-center transition-colors group"
        >
          <Upload className="w-8 h-8 text-slate-500 group-hover:text-green-400 mx-auto mb-2 transition-colors" />
          <p className="text-slate-300 font-medium">{fileName || 'Click to select file'}</p>
      ...
      {step !== 'upload' && step !== 'done' && (
        <div className="bg-[#1e293b] rounded-2xl border border-[#334155] overflow-hidden">
          <div className="px-5 py-4 border-b border-[#334155] flex items-center justify-between">
            <div>
              <h2 className="text-white font-semibold">Preview ({rows.length} rows)</h2>
          ...
          {parseErrors.length > 0 && (
            <div className="px-5 py-3 bg-red-500/5 border-b border-[#334155]">
          ...
          <div className="overflow-x-auto max-h-96">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={i} className="border-b border-[#334155] last:border-0 hover:bg-[#0f172a]/40">
                    <td className="px-4 py-2.5 text-slate-400">{row.member_id ?? 'Auto'}</td>
                    <td className="px-4 py-2.5 text-white font-medium">{row.full_name}</td>
                    <td className="px-4 py-2.5 text-slate-400">{row.mobile}</td>
                    <td className="px-4 py-2.5 text-slate-400">{row.join_date ?? '—'}</td>
                    <td className="px-4 py-2.5 text-slate-400">{row.plan_name ?? '—'}</td>
                    <td className="px-4 py-2.5 text-slate-400">{row.expiry_date ?? '—'}</td>
                    <td className="px-4 py-2.5 text-slate-400">{row.amount_paid ? `₹${row.amount_paid}` : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {/* Done */}
      {step === 'done' && result && (
        <div className={`rounded-2xl border p-6 ${result.errors.length === 0 ? 'bg-green-500/10 border-green-500/30' : 'bg-[#1e293b] border-[#334155]'}`}>
          <div className="flex items-center gap-3 mb-3">
            <CheckCircle className="w-6 h-6 text-green-400" />
            <h2 className="text-white font-semibold">Import Complete</h2>
          </div>
          <p className="text-green-400 font-medium">{result.imported} members imported successfully</p>
