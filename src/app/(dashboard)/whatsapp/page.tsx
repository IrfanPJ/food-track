...
function MemberReminderRow({ member, type }: { member: DueMember; type: 'due_today' | 'due_in_3_days' | 'expired' }) {
  ...
  return (
    <div className={`flex items-center justify-between p-4 rounded-xl border ${sent ? 'bg-green-500/5 border-green-500/20' : 'bg-[#0f172a] border-[#334155]'}`}>
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="text-white font-medium text-sm">{member.full_name}</div>
  ...
  return (
    <div className="bg-[#1e293b] rounded-2xl border border-[#334155] overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-[#334155]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs">{members.length} member{members.length !== 1 ? 's' : ''}</p>
          </div>
        </div>
        {members.length > 0 && (
          <button onClick={handleSendAll} disabled={allSent}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${allSent ? 'bg-green-500/20 text-green-400 cursor-default' : 'bg-green-500 hover:bg-green-600 text-white'}`}>
      ...
      <div className="p-4 space-y-2">
        {members.length === 0 ? (
          <div className="text-center py-6 text-slate-500 text-sm">{emptyText}</div>
        ) : (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-green-300 text-sm font-medium">How WhatsApp reminders work</p>
      ...
      {loading ? (
        <div className="space-y-4">
          {[1,2,3].map(i => <div key={i} className="bg-[#1e293b] rounded-2xl border border-[#334155] h-40 animate-pulse" />)}
        </div>
      ) : (
        <div className="space-y-4">
          <Section
