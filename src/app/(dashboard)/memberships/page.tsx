  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#1e293b] border border-[#334155] rounded-2xl shadow-2xl p-6">
        ...
        <h2 className="text-lg font-semibold text-white mb-5">Add Membership Plan</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 border border-[#334155] hover:border-[#475569] text-slate-400 hover:text-white rounded-xl text-sm transition-colors">Cancel</button>
            <button type="submit" disabled={isPending} className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-green-500 hover:bg-green-400 disabled:opacity-60 text-white rounded-xl text-sm font-semibold transition-colors">
...
function EditPlanForm({ plan, onDone }: { plan: MembershipPlan; onDone: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 pt-4 border-t border-[#334155] space-y-3">
      ...
      <label className="flex items-center gap-2 cursor-pointer select-none">
        <div
          ...
          className={`w-9 h-5 rounded-full relative transition-colors ${isActive ? 'bg-green-500' : 'bg-[#334155]'}`}
        >
      ...
      <div className="flex gap-2">
        <button type="button" onClick={onDone} className="flex-1 px-3 py-2 border border-[#334155] hover:border-[#475569] text-slate-400 hover:text-white rounded-xl text-xs transition-colors">Cancel</button>
        <button type="submit" disabled={isPending} className="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 bg-green-500 hover:bg-green-400 disabled:opacity-60 text-white rounded-xl text-xs font-semibold transition-colors">
    ...
    <div className="bg-[#1e293b] rounded-2xl border border-[#334155] p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-2xl font-bold text-green-400 mt-2">{formatCurrency(plan.fee)}</p>
        ...
        <button
          ...
          className="flex-shrink-0 w-8 h-8 rounded-lg bg-[#334155] hover:bg-[#475569] text-slate-400 hover:text-white flex items-center justify-center transition-colors"
        >
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs">{plans.length} plan{plans.length !== 1 ? 's' : ''}</p>
          </div>
        </div>
        <button
      ...
      {loading ? (
        <div className="bg-[#1e293b] rounded-2xl border border-[#334155] p-16 text-center">
          ...
          <p className="text-slate-400 text-sm mt-3">Loading plans…</p>
        </div>
      ) : plans.length === 0 ? (
        <div className="bg-[#1e293b] rounded-2xl border border-[#334155] p-16 text-center">
          ...
          <p className="text-slate-500 text-sm mt-1">Create your first membership plan above.</p>
        </div>
      ) : (
