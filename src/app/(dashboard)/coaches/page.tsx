  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#1e293b] border border-[#334155] rounded-2xl shadow-2xl p-6">
        ...
        <h2 className="text-lg font-semibold text-white mb-5">Add Coach</h2>
  ...
  return (
    <div className="bg-[#1e293b] rounded-2xl border border-[#334155] p-5">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {coach.specialization && (
            <p className="text-sm text-green-400 mt-0.5">{coach.specialization}</p>
          )}
      ...
      <button
        ...
        className="mt-4 w-full flex items-center justify-center gap-2 px-4 py-2 bg-[#0f172a] hover:bg-[#334155] border border-[#334155] hover:border-[#475569] text-slate-400 hover:text-white rounded-xl text-xs font-medium transition-colors"
      >
      ...
      {expanded && (
        <div className="mt-3 border-t border-[#334155] pt-3">
          {membersLoading ? (
            ...
            <p className="text-slate-500 text-xs text-center py-2">No members assigned yet.</p>
          ) : (
            <div className="space-y-2">
              {members.map((m) => (
                <div key={m.id} className="flex items-center justify-between text-xs bg-[#0f172a] rounded-lg px-3 py-2">
                  <span className="text-white font-medium">{m.full_name}</span>
                  <span className="text-slate-500">#{m.member_id} · {m.mobile}</span>
                </div>
              ))}
            </div>
          )}
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs">{coaches.length} coach{coaches.length !== 1 ? 'es' : ''}</p>
          </div>
        </div>
        <button
      ...
      {loading ? (
        <div className="bg-[#1e293b] rounded-2xl border border-[#334155] p-16 text-center">
          ...
          <p className="text-slate-400 text-sm mt-3">Loading coaches…</p>
        </div>
      ) : coaches.length === 0 ? (
        <div className="bg-[#1e293b] rounded-2xl border border-[#334155] p-16 text-center">
          ...
          <p className="text-white font-semibold">No coaches yet</p>
          <p className="text-slate-500 text-sm mt-1">Add your first coach using the button above.</p>
        </div>
      ) : (
